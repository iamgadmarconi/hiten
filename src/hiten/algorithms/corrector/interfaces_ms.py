"""Multiple shooting interface for correction algorithms.

This module implements the multiple shooting method by transforming periodic
orbit problems into augmented systems with continuity constraints. The interface
bridges domain objects (PeriodicOrbit) and the generic correction backend.

The key innovation of multiple shooting is dividing a trajectory into N segments
(patches) and treating each patch's initial state as an independent variable.
Continuity constraints ensure the trajectory is continuous across patch boundaries.

Key Features
------------
- Automatic patch initialization from trajectory sampling
- Block-structured Jacobian assembly from STMs
- Flexible continuity constraint specification
- Robust handling of event-based and fixed-time boundaries

See Also
--------
:class:`~hiten.algorithms.corrector.interfaces._PeriodicOrbitCorrectorInterface`
    Single shooting interface for comparison.
:class:`~hiten.algorithms.corrector.backends.ms._MultipleShootingBackend`
    Backend that solves the augmented system.
"""
...
from hiten.algorithms.corrector.config import _MultipleShootingOrbitCorrectionConfig
