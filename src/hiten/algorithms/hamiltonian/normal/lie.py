...
def _lie_transform(
    ...
    small_divisor_tol : float, optional
        Tolerance for identifying small divisors, default is 1e-10
    resonance_tol : float, optional
        Tolerance for identifying resonant terms, default is 1e-12
    ...
    for n in range(3, max_degree+1):
        ...
        # Select non-resonant terms for elimination using full resonance condition
        p_elim, resonant_indices = _select_nonresonant_terms(
            p_n, n, omega, clmo, resonance_tol
        )
        ...
        p_G_n, small_divs = _solve_homological_equation_full(
            p_elim, n, omega, clmo, small_divisor_tol
        )
        ...
        p_check, _ = _select_nonresonant_terms(
            poly_trans[n], n, omega, clmo, resonance_tol
        )
        if p_check.any():
...
def _select_nonresonant_terms(
    ...
    omega: np.ndarray,
    clmo: np.ndarray,
    resonance_tol: float = 1e-12) -> tuple[np.ndarray, List[int]]:
    r"""
    ...
    tuple[numpy.ndarray, List[int]]
        ...
        - List of indices of resonant terms (preserved in normal form)
        
    Notes
    -----
    A term with multi-index k is resonant if (k,ω) = 0, where:
    (k,ω) = k₀ω₁ + k₁ω₂ + k₂ω₃ + k₃(-ω₁) + k₄(-ω₂) + k₅(-ω₃)
    
    This is equivalent to:
    (k,ω) = (k₀-k₃)ω₁ + (k₁-k₄)ω₂ + (k₂-k₅)ω₃
    """
    p_elim = p_n.copy()
    resonant_indices = List()
    resonant_indices.append(-1)  # Placeholder, will be removed
    
    for i in range(p_n.shape[0]):
        if p_elim[i] != 0.0:
            k = _decode_multiindex(i, n, clmo)
            
            # Compute resonance condition (k,ω) with correct sign pattern
            # For 6D phase space: (k₃-k₀)ω₁ + (k₄-k₁)ω₂ + (k₅-k₂)ω₃
            resonance_value = ((k[3] - k[0]) * omega[0] + 
                             (k[4] - k[1]) * omega[2] + 
                             (k[5] - k[2]) * omega[4])
            
            if abs(resonance_value) < resonance_tol:
                # This is a resonant term - keep it in normal form
                p_elim[i] = 0.0
                resonant_indices.append(i)
    
    # Remove placeholder
    resonant_indices.pop(0)
    
    return p_elim, resonant_indices
