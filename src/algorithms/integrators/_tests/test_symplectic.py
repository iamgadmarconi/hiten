...
def test_energy_conservation(symplectic_test_data):
    H_poly, jac_H, psi, clmo = symplectic_test_data
    ...
    initial_energy = evaluate_hamiltonian(H_poly, trajectory[0], psi, clmo)
    final_energy = evaluate_hamiltonian(H_poly, trajectory[-1], psi, clmo)
...
def test_reversibility(symplectic_test_data):
    _, jac_H, _, clmo = symplectic_test_data
...
def test_vs_solve_ivp(symplectic_test_data):
    ...
    for i in range(len(actual_times)):
        ...
        scipy_energy.append(evaluate_hamiltonian(H_poly, current_scipy_state_6d, psi, clmo))

        current_analytical_state_6d = np.array([analytical_Q[i], 0.0, 0.0, analytical_P[i], 0.0, 0.0])
        analytical_energy.append(evaluate_hamiltonian(H_poly, current_analytical_state_6d, psi, clmo))
    
    symplectic_energy = []
    for i in range(len(actual_times)):
        state_6d = symplectic_traj[i]
        symplectic_energy.append(evaluate_hamiltonian(H_poly, state_6d, psi, clmo))
