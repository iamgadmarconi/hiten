...
# Create encode dict for Poisson bracket tests
ENCODE_DICT_PB = _create_encode_dict_from_clmo(CLMO_PB)
...
def test_polynomial_zero_list():
    ...
    assert len(poly_list) == PSI_GLOBAL.shape[1]  # Corrected: len should equal max_deg + 1
...
def test_polynomial_variable():
    ...
    for var_idx in range(N_VARS):
        poly = polynomial_variable(var_idx, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
        ...
        for d in range(max_deg_local + 1):
            if d == 1:
                ...
                k[var_idx] = 1
                idx = encode_multiindex(k, 1, ENCODE_DICT_GLOBAL) # Use global ENCODE_DICT_GLOBAL
...
def test_polynomial_variables_list():
    ...
    max_deg_local = PSI_GLOBAL.shape[1]-1
    var_polys = polynomial_variables_list(max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    ...
    for i, poly in enumerate(var_polys):
        # Should be a list of arrays with the same length as initialized by polynomial_variable
        assert len(poly) == PSI_GLOBAL.shape[1]  # This matches the actual length returned
...
def test_polynomial_multiply():
    ...
    # Multiply x * y
    result = polynomial_multiply(x_poly, y_poly, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    ...
    k[0] = 1  # x
    k[1] = 1  # y
    idx = encode_multiindex(k, 2, ENCODE_DICT_GLOBAL) # Use global ENCODE_DICT_GLOBAL
    ...
    trunc_deg = 3
    if (PSI_GLOBAL.shape[1]-1) < 2: # x_squared/y_squared are degree 2
        pytest.skip(f"Global MAX_DEGREE ({PSI_GLOBAL.shape[1]-1}) too small for base of x_squared/y_squared in truncation test")

    x_squared = polynomial_multiply(x_poly, x_poly, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    y_squared = polynomial_multiply(y_poly, y_poly, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    
    # x^2 * y^2 would be degree 4, but we'll truncate to degree 3
    truncated = polynomial_multiply(x_squared, y_squared, trunc_deg, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    ...
    xy = polynomial_multiply(x_poly, y_poly, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    yx = polynomial_multiply(y_poly, x_poly, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
...
def test_polynomial_power():
    ...
    # Test x^0 = 1
    x_pow_0 = polynomial_power(x_poly, 0, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    ...
    # Test x^1 = x
    x_pow_1 = polynomial_power(x_poly, 1, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    
    for d in range(MAX_DEGREE + 1):
        np.testing.assert_array_almost_equal(x_pow_1[d], x_poly[d])
    
    # Test x^2
    x_pow_2 = polynomial_power(x_poly, 2, max_deg_local, PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL) # Pass ENCODE_DICT_GLOBAL
    ...
    trunc_deg = 3
    # Ensure global max_deg is sufficient for intermediate x_poly for power calculation if power itself truncates
    # x_poly uses PSI_GLOBAL, CLMO_GLOBAL, ENCODE_DICT_GLOBAL from its creation
    # The polynomial_power function will then use the max_deg = trunc_deg for its output.
    if (PSI_GLOBAL.shape[1]-1) < 1: # x_poly is degree 1
        pytest.skip(f"Global MAX_DEGREE ({PSI_GLOBAL.shape[1]-1}) too small for base x_poly of power test")
...
def test_complex_polynomials():
    ...
    idx_prod_x0y = encode_multiindex(k_x0y, 2, ENCODE_DICT_GLOBAL) # degree is 1+1=2
    ...
    idx_prod_x0z = encode_multiindex(k_x0z, 2, ENCODE_DICT_GLOBAL) # degree is 1+1=2
...
def test_polynomial_multiply_with_zero_components():
    ...
    # Output max degree is max_deg_local
...
def test_polynomial_poisson_antisymmetry():
    ...
    P = polynomial_variable(0, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # P = x0
    Q = polynomial_variable(3, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Q = p0 (x3)

    PQ = polynomial_poisson_bracket(P, Q, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    QP = polynomial_poisson_bracket(Q, P, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_poisson_linearity_first_arg():
    ...
    P = polynomial_variable(0, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # x0
    Q = polynomial_variable(1, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # x1
    R = polynomial_variable(3, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # p0 (x3)
    ...
    # LHS: {aP+bQ, R}
    lhs = polynomial_poisson_bracket(aPbQ, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB

    # RHS: a{P,R} + b{Q,R}
    PR = polynomial_poisson_bracket(P, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    QR = polynomial_poisson_bracket(Q, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_poisson_linearity_second_arg():
    ...
    P = polynomial_variable(0, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # x0
    Q = polynomial_variable(3, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # p0 (x3)
    R = polynomial_variable(4, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # p1 (x4)
    ...
    # LHS: {P, aQ+bR}
    lhs = polynomial_poisson_bracket(P, aQbR, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_poisson_jacobi_identity():
    ...
    P = polynomial_variable(0, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # x0
    Q = polynomial_variable(3, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # p0 (x3)
    R = polynomial_variable(1, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # x1

    # {Q,R}
    QR = polynomial_poisson_bracket(Q, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # {P,{Q,R}}
    P_QR = polynomial_poisson_bracket(P, QR, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB

    # {R,P}
    RP = polynomial_poisson_bracket(R, P, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # {Q,{R,P}}
    Q_RP = polynomial_poisson_bracket(Q, RP, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB

    # {P,Q}
    PQ = polynomial_poisson_bracket(P, Q, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # {R,{P,Q}}
    R_PQ = polynomial_poisson_bracket(R, PQ, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_poisson_leibniz_rule():
    ...
    # Q*R
    QR_prod = polynomial_multiply(Q, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # LHS: {P, Q*R}
    lhs = polynomial_poisson_bracket(P, QR_prod, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB

    # {P,Q}
    PQ_br = polynomial_poisson_bracket(P, Q, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # {P,Q}*R
    PQ_br_R = polynomial_multiply(PQ_br, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    
    # {P,R}
    PR_br = polynomial_poisson_bracket(P, R, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    # Q*{P,R}
    Q_PR_br = polynomial_multiply(Q, PR_br, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_poisson_constant_property():
    ...
    # {C,P}
    CP_br = polynomial_poisson_bracket(C_poly, P, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
    zero_poly_list = polynomial_zero_list(MAX_DEGREE_PB_TESTS, PSI_PB)
    assert_poly_lists_almost_equal(CP_br, zero_poly_list, msg="{C,P} != 0 failed")

    # {P,C}
    PC_br = polynomial_poisson_bracket(P, C_poly, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
...
def test_polynomial_canonical_relations():
    ...
    q_vars = [polynomial_variable(i, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) for i in range(3)] # x0,x1,x2
    p_vars = [polynomial_variable(i+3, MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) for i in range(3)] # x3,x4,x5 (p0,p1,p2)
    ...
    for i in range(3):
        for j in range(3):
            # {q_i, q_j}
            qi_qj_br = polynomial_poisson_bracket(q_vars[i], q_vars[j], MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
            assert_poly_lists_almost_equal(qi_qj_br, zero_poly_list, msg=f"{{q{i},q{j}}} != 0")

            # {p_i, p_j}
            pi_pj_br = polynomial_poisson_bracket(p_vars[i], p_vars[j], MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
            assert_poly_lists_almost_equal(pi_pj_br, zero_poly_list, msg=f"{{p{i},p{j}}} != 0")

            # {q_i, p_j}
            qi_pj_br = polynomial_poisson_bracket(q_vars[i], p_vars[j], MAX_DEGREE_PB_TESTS, PSI_PB, CLMO_PB, ENCODE_DICT_PB) # Pass ENCODE_DICT_PB
            if i == j:
